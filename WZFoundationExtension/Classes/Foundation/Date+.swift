...
public extension Date {
    ...
    /// 获取当前星期几
    func getCurrentWeekType() -> DateWeekType {
        let weeks: [DateWeekType] = [.sunday, .monday, .tuesday, .wednesday, .thursday, .friday, .saturday]
        return weeks[component(.weekday)!-1]
    }
...
public enum DateFormatType {
    ...
    var stringFormat:String {
        ...
        case .nomar: return "yyyy-MM-dd HH:mm:ss"
        case .timeDivision: return "HH:mm"
        case .month: return "MM"
        case .day: return "dd"
        case .hours: return "HH"
        case .points: return "mm"
...
/// MARK - 星期
public enum DateWeekType: String {
    case sunday = "星期日"
    case monday = "星期一"
    case tuesday = "星期二"
    case wednesday = "星期三"
...
public extension NamespaceWrappable where Base == Date {
    ...
    /// - Parameter stringTime: 时间戳
    /// - Parameter formatterStr: 时间格式
    static func timeStampToString(timeStamp: String, type: DateFormatType = .nomar) ->String {
        let string = NSString(string: timeStamp)
        let timeSta: TimeInterval = string.doubleValue
        let dfmatter = DateFormatter()
        dfmatter.dateFormat = type.stringFormat
        let date = Date(timeIntervalSince1970: timeSta)
        return dfmatter.string(from: date)
    }
    
    /// 时间格式转换
    static func timeFormat(stringTime: String, type: DateFormatType = .nomar) -> String {
        return Date.wz.timeStampToString(timeStamp: stringTime, type: type)
    }
    ...
    var messageDate: String {
        ...
        /// 是否今天
        if base.compare(.isToday) {
            return Date.wz.timeStampToString(timeStamp: "\(base.wz.timeStamp)", type: .timeDivision)
        }
        
        /// 是否昨天
        if base.compare(.isYesterday) {
            return "昨天" + Date.wz.timeStampToString(timeStamp: "\(base.wz.timeStamp)",  type: .timeDivision)
        }
        
        /// 是否本周
        if base.compare(.isThisWeek) {
            return base.getCurrentWeekType().rawValue
        }
        
        /// 日期展示具体日期
        return Date.wz.timeStampToString(timeStamp: "\(base.wz.timeStamp)", type: .isoDate)
    }
    
    /// 获取年月日等
    func component(type: DateComponentType) -> Int {
        return base.component(type) ?? 0
    }
    
    ///  秒转小时 00
    static func transToHour(_ time: Int64) -> String {
       let hours = time / 3600
       return hours > 9 ? "\(hours)" : "0\(hours)"
    }
    
    /// 秒转分钟
    static func transToMinutes(_ time: Int64) -> String {
       let minutes = time % 3600 / 60
       return minutes > 9 ? "\(minutes)" : "0\(minutes)"
    }
    
    /// 秒
    static func transToSeconds(_ time: Int64) -> String {
